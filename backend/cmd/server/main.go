...
// Caches the base HTML at startup. Injects dynamic tags by replacing a placeholder comment,
...
func ogMetaHandler(root fs.FS, s store.Store) http.HandlerFunc {
	...
	// Split index.html into before/after the static OG tags so we can replace them.
	// We inject dynamic tags right after <head> and before existing static OG tags.
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if id != "" && s != nil {
			...
			if err == nil {
				...
				// Replace static OG tags with dynamic ones (scrapers use first occurrence)
				html = strings.Replace(html, `<meta property="og:title"`, ogTags+`<meta property="og:title"`, 1)
...
func buildOGMetaTags(pasta *store.Pasta, r *http.Request) string {
    ...
    `, escapeAttr(title), escapeAttr(description), escapeAttr(canonicalURL), escapeAttr(title), escapeAttr(description))
}

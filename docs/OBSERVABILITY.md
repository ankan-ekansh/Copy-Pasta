...
- If `METRICS_TOKEN` env var is set, `/metrics` requires `Authorization: Bearer <token>` header (returns 401 otherwise)
- In production (Azure), the deploy script auto-generates a token shared between the app and Prometheus
- Local dev: leave `METRICS_TOKEN` unset for open access (simpler debugging)
...
| `METRICS_TOKEN` | (not set = open) | If set, `/metrics` requires `Authorization: Bearer <token>` header. Auto-generated by deploy script. |
...
|-------------|----------|
| **Local dev** | Set `EXPOSE_METRICS=true` in `.env` when running with `--profile observability`. No token needed (open access). |
| **Azure prod** | `EXPOSE_METRICS=true` + `METRICS_TOKEN=<auto-generated>`. Deploy script generates a shared token; Prometheus sends it via `authorization.credentials_file`. Public requests to `/metrics` get 401 Unauthorized. |

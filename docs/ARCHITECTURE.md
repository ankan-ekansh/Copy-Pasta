...
| useRef for in-flight guards | Synchronous check prevents double-clicks without waiting for React re-render. `publishingRef` (Set of pasta IDs) gates toggle; `handleTogglePublic` returns boolean so callers know if action executed. |

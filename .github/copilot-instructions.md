...
- **Never run infrastructure scripts (`infra/setup-azure.sh`) or mutating `az` CLI commands without explicit user permission.** Only non-sensitive read-only az commands (e.g., `az account show`, `az containerapp show`, `az group exists`) are allowed autonomously. Commands that reveal secrets or credentials (e.g., `az containerapp secret show`, `az acr credential show`, `az storage account keys list`) also require explicit permission.
